...
`history_mapping.csv`는 로컬 `CHG-*`를 공통 `change_history`에 이관하기 위한 의미 매핑표다. `target_record_id`는 UUID/FK 정책 확정 전까지 비워 두며, 변경 전·후 원문은 `change_history.csv`의 `old_value`·`new_value`를 기준으로 보존한다. 페이지와 섹션은 `history_mapping.csv`에 기록하고, `source_text_reference`로 원문 행을 연결한다.
