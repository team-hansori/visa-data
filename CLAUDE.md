...
담당자별로 비자 유형을 나눠 작업한다(예: E-7-4R 지역특화 숙련기능인력). 각 담당자는 `extraction/`(예정) 아래에서 공고문·심사표를 원문 근거와 함께 근거표로 옮기고, 검수 후에만 공통 스키마 SQL(`seeds/`, `output/`, 예정)로 반영한다. 세부 작업 절차(근거표 컬럼, 상태 코드, 변경 이력 형식 등)는 논의 후 별도 문서로 확정한다.
...
### 역할 분리 (현재)
- 근거표 추출/정규화: `extraction/<비자유형>/` (예정, 담당자별 폴더)
- 검수 완료 데이터: `seeds/` (예정)
