...
- [x] 모든 공통 행에 필요한 출처·페이지·유효기간이 존재한다.
- [x] 원천 ID와 공통 UUID가 분리되고 매핑표로 추적된다.
- [x] FK·UUID·enum·헤더·쿼터·누락·순환 검증 테스트가 통과한다.
- [x] v1→v2 변환 규칙과 비자별 이관 결과·보류 항목이 문서화된다.

마지막 미완료 항목이었던 D-2 부트스트랩 행은 신청 방법을 충북보건과학대학교 2026학년도 모집요강
p.1의 광역비자 제출 절차로 반영하고, `valid_from`은 충청북도 선정 결과 통보일인 2025-04-02로
확정해 닫았다. `known_validation_gaps.txt`의 검증 실패는 0건이다.
