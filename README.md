...
| 🧾 근거 추적 | 🧠 논리 보존 | 🛡️ 안전한 판정 | 🔄 지속 가능한 갱신 |
|---|---|---|---|
| 모든 핵심 값을 공식 문서·페이지·검증일과 연결 | 중첩 AND/OR, 점수, 예외조건을 계산 가능한 구조로 표현 | 미확인 데이터와 AI 추측을 자동 판정에서 차단 | 공고 차수별 변경을 추적하고 트랜잭션으로 재배포 |

공식 문서를 단순히 요약하는 것이 아니라, **어떤 문서의 어느 근거로 어떤 결과가 만들어졌는지 설명할 수 있는 데이터**를 만드는 것이 이 프로젝트의 핵심입니다.
...
| 기능 | 설명 | 대표 산출물 |
|---|---|---|
| 다형식 원문 추출 | PDF·HWPX·구버전 HWP의 문단과 표를 재사용 가능한 텍스트로 변환 | `extract_pdf.py`, `extract_hwpx.py`, `extract_hwp.py` |
| 비자요건 구조화 | 자격조건의 값·단위·비교연산자·적용기간·AND/OR 관계를 분리 | `visa_criterion_groups`, `visa_requirement_criteria` |
| 점수·서류·절차 모델링 | 점수 구간, 필수 최소점수, 신청 단계와 제출서류 관계를 표현 | `visa_scoring_*`, `visa_process_stages`, `document_requirements` |
| 공고 변경 추적 | 이전·현재 차수 원문을 비교해 추가·삭제·값·범위 변경을 기록 | `change_history`, `diff_announcement_rounds.py` |
| 출처 계보 관리 | 원천 행에서 공통 서비스 레코드까지 변환 경로를 추적 | `source_documents`, `source_record_mappings` |
| 자동 무결성 검증 | UUID·FK·순환참조·논리구조·쿼터 산술·회귀를 검사 | 407개 pytest 테스트, 전용 검증 스크립트 |
| 안전한 DB 적재 | 검증 완료 데이터를 단일 트랜잭션으로 Supabase에 동기화 | migration, 멱등 importer, 실패 시 전체 rollback |
...
```mermaid
flowchart TB
    subgraph SOURCES[공식 원천 계층]
        A1[충청북도 공고문·붙임]
        A2[법무부·공공기관 안내]
        A3[대학 공식 모집요강]
    end

    subgraph PIPELINE[추출·정규화 계층]
        B1[PDF · HWPX · HWP 추출]
        B2[요건 · 점수 · 절차 · 서류 · 쿼터 분리]
        B3[차수별 변경 비교]
    end

    subgraph QUALITY[검수·품질 계층]
        C1{원문 대조 및 사람 검수}
        C2[검토 큐 · 소비 차단]
        C3[UUID · FK · 논리 · 산술 검증]
    end

    subgraph DATA[서비스 데이터 계층]
        D1[(공통 스키마 v2<br/>13개 테이블)]
