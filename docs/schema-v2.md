...
2. **서비스 테이블 10개와 지원 테이블 3개의 실제 헤더가 명세와 일치한다.** — 완료.
   `validate_directory()`가 13개 파일의 헤더 순서를 스키마와 대조하며, 현재 알려진 1건의
...
14. **모든 공통 행에 필요한 출처·페이지·유효기간이 존재한다.** — 부분 완료. 알려진 위반이
    1건 있다. D-2의 정체성·지역·프로그램 유형은 2025년 충청북도 안내자료로 확인했고,
    신청 방법은 충북보건과학대학교 2026학년도 모집요강 p.1의 광역비자 제출 절차를 근거로
    `application_method`에 반영했다. 일 단위 시행일은 공식 자료에서 확인되지 않아
    `valid_from`만 공란으로 유지한다. `source_documents.csv`의
    `f4r_r12_announcement` 원본 파일(`.hwpx`, 12차 공고문)은 taeeunni가 확보해 로컬
    저장소(`data/raw/지역특화_재외동포_F-4-R/`)에 추가했고, `source_location`을 채워
    이 항목의 위반은 닫혔다.
    `uv run python scripts/validate_common_schema_v2.py`는 지금도 이 1건을 그대로
    보고한다(억지로 통과시키지 않음).
...
16. **FK·UUID·enum·헤더·쿼터·누락·순환 검증 테스트가 통과한다.** — 완료. 13개 테이블의
    ...
    알려진 1건과 정확히 일치할 때만 baseline 모드에서 통과하며, 새 오류나 조용한 해소 모두를
...
- **D-2 부트스트랩 필수값 1건**: 2025년 충청북도 외국인정책 지원사업 안내 p.7에서 확인 가능한
  정체성·지역·프로그램 유형을 반영하고, 충북보건과학대학교 2026학년도 모집요강 p.1에서 확인한
  대학 제출 → 관할 출입국사무소 사증발급인정 신청 절차를 `application_method`로 반영했다.
  정확한 일 단위 시행일만 확인되지 않아 `known_validation_gaps.txt`에서 추적한다.
