...
2. **서비스 테이블 10개와 지원 테이블 3개의 실제 헤더가 명세와 일치한다.** — 완료.
   `validate_directory()`가 13개 파일의 헤더 순서를 스키마와 대조하며, 현재 알려진 2건의
...
14. **모든 공통 행에 필요한 출처·페이지·유효기간이 존재한다.** — 부분 완료. 알려진 위반이
    2건 있다. D-2의 정체성·지역·프로그램 유형과 2025년 안내자료 출처·페이지·검증일은
    원문 확인 후 채웠지만, 안내자료에는 신청 방법과 일 단위 시행일이 없어
    `application_method`와 `valid_from`은 공란으로 유지한다. `source_documents.csv`의
    `f4r_r12_announcement` 원본 파일(`.hwpx`, 12차 공고문)은 taeeunni가 확보해 로컬
    저장소(`data/raw/지역특화_재외동포_F-4-R/`)에 추가했고, `source_location`을 채워
    이 항목의 위반은 닫혔다.
    `uv run python scripts/validate_common_schema_v2.py`는 지금도 이 2건을 그대로
    보고한다(억지로 통과시키지 않음).
15. **원천 ID와 공통 UUID가 분리되고 매핑표로 추적된다.** — 완료.
    ...
    139행은 의도적 SKIP, 1행만 실제 MANUAL_REVIEW다.
...
16. **FK·UUID·enum·헤더·쿼터·누락·순환 검증 테스트가 통과한다.** — 완료. 13개 테이블의
    ...
    알려진 2건과 정확히 일치할 때만 baseline 모드에서 통과하며, 새 오류나 조용한 해소 모두를
...
- **D-2 부트스트랩 필수값 2건**: 2025년 충청북도 외국인정책 지원사업 안내 p.7에서 확인 가능한
  정체성·지역·프로그램 유형과 출처 필드는 반영했다. 공식 신청 방법과 정확한 일 단위 시행일은
  해당 자료에서 확인되지 않아 공란이며, `known_validation_gaps.txt`에서 추적한다.
