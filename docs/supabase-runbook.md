...
common_schema_v2의 13개 테이블과 reference 스키마의 3개 테이블(`agency_contacts`/
`risk_routing_table`/`risk_keyword_messages`) + `map_visible_agency_contacts` VIEW 이외의
기존 객체를 삭제하거나 변경하면 중단한다.
...
`agency_contacts`/`risk_routing_table`/`risk_keyword_messages`(`reference/` 디렉터리 CSV)는
별도 importer `scripts/import_reference_data.py`로 적재한다. dry-run은 `validate_fk_integrity.py`
preflight와 예상 행 수만 확인하고 DB에 연결하지 않는다.

```bash
uv run python scripts/import_reference_data.py
```

`import_common_v2.py`와 동일하게 비밀번호를 셸 기록에 직접 넣지 않고 환경 변수로 주입한 뒤
`--apply`로 실제 적재한다.

```bash
read -s DATABASE_URL
export DATABASE_URL
uv run python scripts/import_reference_data.py --apply
unset DATABASE_URL
```

이 importer는 preflight로 `validate_fk_integrity.py`를 실행한 뒤 단일 트랜잭션에서
PK(`risk_keyword_messages`는 복합 PK `(keyword_category, resolution_type)`) 기준 upsert하고,
"CSV의 모든 PK가 DB에 존재하는가"만 검증한다 — `agency_contacts.is_active=false`로 폐쇄를
표현하는 설계와 일치시키기 위해 CSV에서 사라진 행을 자동 삭제하지 않으므로,
`import_common_v2.py`처럼 "DB 행 수 == CSV 행 수"를 기대하지 않는다. `map_visible_agency_contacts`
VIEW는 별도 적재 대상이 아니며 migration 배포 시 함께 생성된다.

## 실패와 복구

- importer 실패: 트랜잭션 전체가 자동 rollback된다. 오류를 수정하고 다시 실행한다.
  `import_common_v2.py`, `import_reference_data.py` 모두 동일하다.
- migration 실패: `supabase migration list`로 적용 여부를 확인한다. 일부 DDL이 적용됐거나
  ...
  사용한다. reference 3개 테이블 + VIEW도 동일한 migration 파일로 배포되므로 복구 절차가
  다르지 않다.
